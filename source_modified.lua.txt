-- Minimalised UI file (white/black, no themes, no icons, no analytics)
...
-- Single static theme (white background, black text)
local SelectedTheme = {
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"UI Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"UI Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('UI had an issue accessing configuration saving capability.')
		end
	end
end
...
local Rayfield = useStudio and script.Parent:FindFirstChild('UI') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	...
	if not warned then
		...
		print('UI may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Rayfield = Rayfield, useStudio and script.Parent:FindFirstChild('UI') or game:GetObjects("rbxassetid://10804731440")[1]
...
-- Remove all corner rounding (UICorner) to make UI square as requested
task.spawn(function()
    pcall(function()
...
local function ChangeTheme(Theme)
    -- Theme switching removed. Single white/black theme is used.
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('UI had an issue decoding the configuration file, please try delete the file and reopen Rayfield.') return end
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			--RayfieldLibrary:Notify({Title = "UI Flags", Content = "Rayfield was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Image then
            -- Icons/images disabled in minimal UI; show empty icon
		...
		if data.Actions then
			warn('UI | Not seeing your actions in notifications?')
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('UI Settings', 0, true)

	if TabList['UI Settings'] then
		TabList['UI Settings'].LayoutOrder = 1000
	end

	if Elements['UI Settings'] then
		Elements['UI Settings'].LayoutOrder = 1000
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RayfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'UI may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		...
		if isfile and not isfile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"UI RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("UI | "..Key.." Error " ..tostring(Response))
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("UI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
	...
	if not success then warn('UI had an issue creating settings.') end
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['UI Settings'])
		end)
	end)
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "UI Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "UI Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('UI Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "UI Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--	Name = "UI Example Window",
	--	LoadingTitle = "UI Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	----RayfieldLibrary:Notify({Title = "UI Interface", Content = "Welcome to Rayfield. These - are the brand new notification design for Rayfield, with custom sizing and Rayfield calculated wait times.", Image = 4483362458})
