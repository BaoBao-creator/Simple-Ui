...
local function makeDraggable(gui, handle)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            ...
            gui.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + 
...
function SimpleUI:CreateWindow(params)
    ...
    function window:CreateTab(name)
        ...
        function tabObj:CreateTextBox(config)
            ...
            local init = config.CurrentValue or ""
            local callback = config.Callback or function() end
            local textContainer = Instance.new("Frame")
            textContainer.Name = "TextBox_"..name
            textContainer.Parent = contentScroll
            textContainer.BackgroundTransparency = 1
            textContainer.Size = UDim2.new(1, 0, 0, 30)
            textContainer.LayoutOrder = contentLayout.AbsoluteContentSize.Y
            -- Tên ở bên trái
            local nameLabel = Instance.new("TextLabel")
        ...
        function tabObj:CreateDropdown(config)
            ...
            selectedBtn.Size = UDim2.new(0.5, 0, 1, -6)
            ...
            refreshBtn.Parent = dropContainer
            refreshBtn.Size = UDim2.new(0.1, 0, 1, -6)
            refreshBtn.Position = UDim2.new(0.9, 0, 0, 3)
            refreshBtn.BackgroundColor3 = Color3.new(0, 0, 0)
            refreshBtn.BorderColor3 = Color3.new(1, 1, 1)
